...
class SearchTag extends Component {
    ...
    loadMessage(component){
        ...
        properties["ts"] = component.state.tagResults[component.state.currentResultIndex].data.msg_ts;
        properties['limit'] = 15
        properties['component'] = component;
    ...
    renderResult(){
        if(this.state.tagResults.length > 0){
            return(
                <div>
                    <button onClick={this.prevResult.bind(this)}>prev</button>
        ...
        if(this.state.currentRoomId){
            return (
                <div >
                    ...
                    <button id='search-button' onClick={this.searchTag.bind(this)}>search</button>
                    {this.renderResult()}
                </div>
            )
        }else{
