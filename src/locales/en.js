module.exports = {
  app: {
    identifier: {
      app_name: 'VNLaw',
      language: 'Language',
      slogan: 'Always make an awesome support',
      or: 'OR',
      vi: 'Vietnamese',
      en: 'English',
      account: 'Account',
      setting: 'Setting',
      home: 'Home',
      login: 'Log in',
      signup: 'Signup',
      logout: 'Log out',
      search: 'Search law',
      profile: 'Profile page',
      login_face: 'Login with facebook',
      login_google: 'Login with google',
      read_more: 'read more',
      show_less: 'show less'
    },
    notification:{
      no_noti: 'No notifcation now',
      new_request_from_other: ' has sended to you a request.',
      click_here: 'Click here for detail!',
      time_created: 'Time created: ',
      detail: 'Details: ',
      create_dialog: 'Create a dialog'
    },
    system_notice:{
      unauthenticated: {
        title: 'Access denied',
        text: 'Please login first!'
      },
      permission_denied:{
        title: 'Access denied',
        text: 'No permission!'
      } 
    },
    home: {
      footer: {
        content: 'VNLaw - LKBC 2017'
      },
      find_box: {
        title: 'Find lawyer',
        lawyer_name: 'Lawyer name',
        lawyer_field: 'Lawyer fields'
      },
      recent_lawyer: {
        title: 'Recent Lawyers',
        lawyer_overview: 'Lawyer overview',
        hour: 'hour',
        apply: 'Make an appointment',
        show_all: 'Show all lawyers',
      },
      get_start: {
        title: 'Start scheduling consultants with lawyers now',
        button_content: 'Get started'
      },
      search_law: {
        title: 'Search Law',
        des: 'In addition to legal advice you can also look at the law with us',
        field_1: {
          title: 'Criminal',
          des: 'Criminal regulations, criminal behavior and penalties'
        },
        field_2: {
          title: 'Intellectual Property',
          des: 'Provisions on copyright, industrial property rights, and the protection of such rights.'
        },
        field_3: {
          title: 'Marriage & Family',
          des: 'Legal provisions on marriage and family relationships'
        },
        field_4: {
          title: 'Land - Construction',
          des: 'Issues related to land, housing, construction'
        },
        field_5: {
          title: 'Financial banking',
          des: 'The provisions of law on organization and operation of credit institutions and other organized banking activities'
        },
        field_6: {
          title: 'Civil',
          des: 'Regulations on property relations and personal relations, inheritance'
        },
        field_7: {
          title: 'Labor - Social Insurance',
          des: 'The provisions of the law on contracts, wages, age, working time'
        },
        field_8: {
          title: 'Enterprise',
          des: 'The provisions of the law on business activities, business investment'
        }
      },
      find_lawyer: 'Talk to lawyer &',
      find_law: 'Look up the law',
      fast: 'fast and most effective',
      find: 'Find',
      schedule: 'Schedule',
      connect: 'Connect',
      next: 'next',
      previous: 'previous',
      find_content: 'Browse through attorney profiles to find the right lawyer for you. All VNLaw attorneys are licensed and active.',
      schedule_content: 'Schedule a free consultation with a local attorney in under 60 seconds today. Instantly view available timeslots in lawyers’ calendars and click to book.',
      connect_content: 'Connect with local attorneys for initial consultations quickly and securely. VNLaw uses the most advanced video chat technology on the planet. Want to meet in-person? We do that too.',
      slogan: 'Compare top-rated lawyers in your area and schedule a consultation in seconds'
    },
    attorney: {
      book: 'Book now',
      view_profile: 'View profile',
      hour: ' / hr',
      online: 'Online',
      away: 'Away',
      rate: 'Rate',
      cost: 'Fee per hour',
      sort_by: 'Sort by',
      search_placeholder: 'Search by attorney\'s name',
      filter: 'Filters',
      specialize: 'Specializes',
      top_view: 'Top view lawyers'
    },
    profile: {
      user_review: 'User review',
      view_all: 'View all',
      review: 'review(s)'
    },
    nav: {
      brow_law: 'Browse Law',
      brow_lawyers: 'Browse Lawyers',
      support: 'Support',
      chat: 'Chat',
      sign_out: 'Sign Out',
      setting: 'Setting'
    },
    login: {
      login_error: 'Password or Email Wrong',
      success: 'Login Success',
      username: 'User name',
      password: 'Password',
      email: 'Email',
      submit: 'Login',
      had_account: 'Already have an account?'
    },
    signup: {
      signup_error: 'Signup infor wrong',
      success: 'Signup Success',
      username: 'User name',
      password: 'Password',
      password_confirmation: 'Password confirmation',
      email: 'Email',
      name: 'Name',
      submit: 'Signup',
      new_to_us: 'New to us?'
    },
    dashboard: {
      home: 'Home',
      customer: 'Customers',
      file: 'Files shared',
      todo: 'Todo',
      todo_list: 'Todo list',
      todo_list_warning: 'You must write something',
      todo_list_none: 'You do not have any work to do!',
      todo_list_title: 'My to do list with ',
      todo_title: 'Working content',
      todo_add: 'Add',
      with: 'With',
      chat_title: 'Chat with',
      dashboard: 'Dashboard',
      calendar: 'Calendar',
      note: 'Notes',
      profile: 'My profile',
      submit_des: 'Submit description',
      edit_des: 'Edit description',
      chat_setting_des_title: 'Chat description',
      search: {
        title: 'Search',
        by_user: 'Search by user',
        by_tag: 'Search by hastag',
        search_user_not_found: 'No users found',
        search_user_not_found_symbol: '=.="',
        input_user_name: 'Please input user\'s name that you want to search',
        search_tag_not_found: 'No results found',
        search_tag_not_found_symbol: '-_-"',
        result_for: ' result(s) for ',
        result_relate: ' related results '
      }
    },
    user: {
      list: 'List users',
      search: 'Search user',
      name: 'User name',
      upload: {
        ava: 'Upload avatar'
      },
      edit: {
        ava: 'Edit avatar',
        name: 'Edit user name',
        profile: 'Edit profile'
      },
      status: {
        online: 'online',
        offline: 'offline',
        away: 'away'
      }
    },
    chat: {
      title: 'Chat',
      input_place_holder: 'Input messages ...',
      my_chat: 'My chat',
      shared_files: 'Shared files',
      shared_images: 'Shared images',
      lawyer_profile: 'Lawyer infor',
      rate: 'Rate',
      pay: 'Pay'
    },
    notfound: {
      content: {
        oh: 'Ohh.....',
        main: 'You Requested the page that is no longer There.',
        back_home: 'Back To Home'
      }
    },
    search: {
      founded: 'Has founded',
      results: 'result(s)',
      not_found_symbol: '>_<',
      has: 'Has',
      you_mean: 'You want to search: ',
      search_tool: {
        title: 'Search',
        key_search: 'Search keywords',
        filter: {
          filter_1: 'Exact phrase above',
          filter_2: 'Have all of the above'
        },
        order_by: {
          title: 'Order by',
          order_by_1: 'Public day',
          order_by_2: 'Effect day',
          order_by_3: 'New to old',
          order_by_4: 'Old to new',
          order_by_5: 'The most exactly'
        }
      },
      category_title: 'Filter by',
      category_organ: {
        title: 'Category Organization',
        organ_1: 'Congress',
        organ_2: 'National Assembly Standing Committee',
        organ_3: 'Goverment',
        organ_4: 'Prime Minister',
        organ_5: 'Ministries and ministerial-level agencies',
        organ_6: 'Other agencies',
      },
      category_type: {
        title: 'Category Type',
        type_1: 'Constitution',
        type_2: 'Laws',
        type_3: 'Law',
        type_4: 'Order',
        type_5: 'Command',
        type_6: 'Resolution',
        type_7: 'Joint Resolution',
        type_8: 'Decree',
        type_9: 'Decided',
        type_10: 'Circulars',
        type_11: 'Joint Circular'
      },
      category_year: {
        title: 'Category Year',
        year_1: {
          title: '1945 to 1950',
          from_year: '1945',
          year_mid_1: '1946',
          year_mid_2: '1947',
          year_mid_3: '1948',
          year_mid_4: '1949',
          to_year: '1950',
        },
        year_2: {
          title: '1951 to 1960',
          from_year: '1951',
          year_mid_1: '1952',
          year_mid_2: '1953',
          year_mid_3: '1954',
          year_mid_4: '1955',
          year_mid_5: '1956',
          year_mid_6: '1957',
          year_mid_7: '1958',
          year_mid_8: '1959',
          to_year: '1960'
        },
        year_3: {
          title: '1961 to 1970',
          from_year: '1961',
          year_mid_1: '1962',
          year_mid_2: '1963',
          year_mid_3: '1964',
          year_mid_4: '1965',
          year_mid_5: '1966',
          year_mid_6: '1967',
          year_mid_7: '1968',
          year_mid_8: '1969',
          to_year: '1970'
        },
        year_4: {
          title: '1971 to 1980',
          from_year: '1971',
          year_mid_1: '1972',
          year_mid_2: '1973',
          year_mid_3: '1974',
          year_mid_4: '1975',
          year_mid_5: '1976',
          year_mid_6: '1977',
          year_mid_7: '1978',
          year_mid_8: '1979',
          to_year: '1980'
        },
        year_5: {
          title: '1981 to 1990',
          from_year: '1981',
          year_mid_1: '1982',
          year_mid_2: '1983',
          year_mid_3: '1984',
          year_mid_4: '1985',
          year_mid_5: '1986',
          year_mid_6: '1987',
          year_mid_7: '1988',
          year_mid_8: '1989',
          to_year: '1990'
        },
        year_6: {
          title: '1991 to 2000',
          from_year: '1991',
          year_mid_1: '1992',
          year_mid_2: '1993',
          year_mid_3: '1994',
          year_mid_4: '1995',
          year_mid_5: '1996',
          year_mid_6: '1997',
          year_mid_7: '1998',
          year_mid_8: '1999',
          to_year: '2000'
        },
        year_7: {
          title: '2001 to 2010',
          from_year: '2001',
          year_mid_1: '2002',
          year_mid_2: '2003',
          year_mid_3: '2004',
          year_mid_4: '2005',
          year_mid_5: '2006',
          year_mid_6: '2007',
          year_mid_7: '2008',
          year_mid_8: '2009',
          to_year: '2010'
        },
        year_8: {
          title: '2011 to 2020',
          from_year: '2011',
          year_mid_1: '2012',
          year_mid_2: '2013',
          year_mid_3: '2014',
          year_mid_4: '2015',
          year_mid_5: '2016',
          year_mid_6: '2017',
          year_mid_7: '2018',
          year_mid_8: '2019'
        }
      }
    },
    payment: {
      title: 'Enter billing information',
      connect: 'Connect to lawyer',
      table_input_info: 'Input table',
      personal_info: 'Personal information',
      full_name: 'Full name',
      placeholder_name: 'Nguyen Tien Truong',
      country: 'Country',
      placeholder_country: 'Ha Noi',
      phone_number: 'Phone number',
      placeholder_phone: '19008198',
      age: 'Age',
      placeholder_age: '19',
      problem: 'Problems encountered',
      apply_lawyer: 'Request an lawyer',
      personal_info_overview: 'The basic personal information will help us understand you better',
      problem_overview: 'Please share with us the problems that you encounterư',
      pre_step: 'Previous step',
      success_deposit: 'Success deposit ',
      fail_deposite: 'Exchange doesn\'t exist',
      fail_deposite_1: 'Bank Declined',
      fail_deposite_3: 'Merchant not exist',
      fail_deposite_4: 'Invalid access code',
      fail_deposite_5: 'Invalid amount',
      fail_deposite_6: 'Invalid currency code',
      fail_deposite_7: 'Unspecified Failure',
      fail_deposite_8: 'Invalid card Number',
      fail_deposite_9: 'Invalid card name',
      fail_deposite_10: 'Expired Card',
      fail_deposite_11: 'Card Not Registed Service',
      fail_deposite_12: 'Invalid card date',
      fail_deposite_13: 'Exist Amount',
      fail_deposite_21: 'Insufficient fund',
      fail_deposite_99: 'User cancel',
      fail_deposite_default: 'Failured'
    },
    lawyer: {
      online_counsel: 'Online Counseling',
      pay_for_hour: 'Pay for hour',
      card_number: 'Card number',
      certificate: 'Certificate',
      experience: 'Year experience',
      birthday: 'Birthday',
      work_place: 'Work place',
      intro: 'Introduction',
      achievement: 'Achievement',
      education: 'Education',
      regis_now: 'Register now'
    },
    customer: {
      account_type: 'Account type:'
    },
    settings: {
      basic_infor: 'Basic Information',
      detail_infor: 'Detail Information',
      avatar: 'Avatar',
      name: 'Name',
      user_name: 'User name',
      birthday: 'Birthday',
      card_number: 'Card number',
      certificate: 'Certificate',
      experience: 'Year experience',
      change_ava_guide: 'Click to change avatar',
      category: 'Category',
      expyear: 'Exp year',
      intro: 'Introduction',
      achievements: 'Achievements',
      education: 'Education',
      work_place: 'Working place',
      save: 'Save',
      acc_balance: 'Account balance',
      money_unit: 'VNĐ',
      input_money: 'Enter the amount you want to load',
      job_done: 'Completed work'
    },
    rate: {
      cost_money: 'Price',
      tips: 'Your feedback will help us serve you better',
      thanks: 'Thanks you !',
      rate_tips: 'Please rate our service',
      rate_done: 'Submit a review' 
    }
  }
}
